...
#include <rosplan_planning_system/ProblemGeneration/CSVGenerator.h>
...
namespace KCL_rosplan {
    ...
    void CSVGenerator::makeHeader(std::ofstream &pFile) {
        ...
        if (!getNameClient.call(nameSrv)) {
            ROS_ERROR("KCL: (CSVGenerator) Failed to call service %s", domain_name_service.c_str());
        }
        ...
        if (!getTypesClient.call(typeSrv)) {
            ROS_ERROR("KCL: (CSVGenerator) Failed to call service %s", domain_type_service.c_str());
        }
        ...
        for(size_t t=0; t<typeSrv.response.types.size(); t++) {
            ...
            if (!getInstancesClient.call(instanceSrv)) {
                ROS_ERROR("KCL: (CSVGenerator) Failed to call service %s: %s", state_instance_service.c_str(), instanceSrv.request.type_name.c_str());
            } else {
    ...
    void CSVGenerator::makeInitialState(std::ofstream &pFile) {
        ...
        if (!getDomainPropsClient.call(domainAttrSrv)) {
            ...
            for(; ait != domainAttrSrv.response.items.end(); ait++) {
                ...
                if (!getPropsClient.call(attrSrv)) {
                    ROS_ERROR("KCL: (CSVGenerator) Failed to call service %s: %s", state_proposition_service.c_str(), attrSrv.request.predicate_name.c_str());
                } else {
                ...
                if (!getTILsClient.call(attrSrv)) {
                    ROS_ERROR("KCL: (CSVGenerator) Failed to call service %s: %s", state_timed_knowledge_service.c_str(), attrSrv.request.predicate_name.c_str());
                } else {
        ...
        if (!getDomainFuncsClient.call(domainAttrSrv)) {
            ROS_ERROR("KCL: (CSVGenerator) Failed to call service %s", domain_function_service.c_str());
        } else {
            ...
            for(; ait != domainAttrSrv.response.items.end(); ait++) {
                ...
                if (!getFuncsClient.call(attrSrv)) {
                    ROS_ERROR("KCL: (CSVGenerator) Failed to call service %s: %s", state_function_service.c_str(), attrSrv.request.predicate_name.c_str());
                } else {
    ...
    void CSVGenerator::makeGoals(std::ofstream &pFile) {
        ...
        if (!getCurrentGoalsClient.call(currentGoalSrv)) {
            ROS_ERROR("KCL: (CSVGenerator) Failed to call service %s", state_goal_service.c_str());
        } else {
    ...
    void CSVGenerator::makeMetric(std::ofstream &pFile) {
        ...
        if (!getCurrentMetricClient.call(currentMetricSrv)) {
            ...
            ROS_ERROR("KCL: (CSVGenerator) Failed to call service %s", state_metric_service.c_str());
    ...
    void CSVGenerator::printExpression(std::ofstream &pFile, rosplan_knowledge_msgs::ExprComposite & e) {
    ...
    void CSVGenerator::makeProblem(std::ofstream &pFile) {
        makeHeader(pFile);
