#include "rosplan_planning_system/ExportInterface/ExportGenerator.h"

namespace KCL_rosplan {

	/**
	 * generates a CSV file.
	 * This file is later read by the prediction interface.
	 */
	void ExportGenerator::generateCSVFile(std::string &problemPath) {
	...
	void ExportGenerator::makeHeader(std::ofstream &pFile) {
		...
		if (!getNameClient.call(nameSrv)) {
			ROS_ERROR("KCL: (ExportGenerator) Failed to call service %s", domain_name_service.c_str());
		}
		...
		if (!getTypesClient.call(typeSrv)) {
			ROS_ERROR("KCL: (ExportGenerator) Failed to call service %s", domain_type_service.c_str());
		}
		...
		for(size_t t=0; t<typeSrv.response.types.size(); t++) {
			...
			if (!getInstancesClient.call(instanceSrv)) {
				ROS_ERROR("KCL: (ExportGenerator) Failed to call service %s: %s", state_instance_service.c_str(), instanceSrv.request.type_name.c_str());
			} else {
	...
	void ExportGenerator::makeInitialState(std::ofstream &pFile) {
		...
		if (!getDomainPropsClient.call(domainAttrSrv)) {
			ROS_ERROR("KCL: (ExportGenerator) Failed to call service %s", domain_predicate_service.c_str());
		} else {
			...
			for(; ait != domainAttrSrv.response.items.end(); ait++) {
				...
				if (!getPropsClient.call(attrSrv)) {
					ROS_ERROR("KCL: (ExportGenerator) Failed to call service %s: %s", state_proposition_service.c_str(), attrSrv.request.predicate_name.c_str());
				} else {
				...
				if (!getTILsClient.call(attrSrv)) {
					ROS_ERROR("KCL: (ExportGenerator) Failed to call service %s: %s", state_timed_knowledge_service.c_str(), attrSrv.request.predicate_name.c_str());
				} else {
		...
		if (!getDomainFuncsClient.call(domainAttrSrv)) {
			ROS_ERROR("KCL: (ExportGenerator) Failed to call service %s", domain_function_service.c_str());
		} else {
			...
			for(; ait != domainAttrSrv.response.items.end(); ait++) {
				...
				if (!getFuncsClient.call(attrSrv)) {
					ROS_ERROR("KCL: (ExportGenerator) Failed to call service %s: %s", state_function_service.c_str(), attrSrv.request.predicate_name.c_str());
				} else {
	...
	void ExportGenerator::makeGoals(std::ofstream &pFile) {
		...
		if (!getCurrentGoalsClient.call(currentGoalSrv)) {
			ROS_ERROR("KCL: (ExportGenerator) Failed to call service %s", state_goal_service.c_str());
		} else {
	...
	void ExportGenerator::makeMetric(std::ofstream &pFile) {
		...
		if (!getCurrentMetricClient.call(currentMetricSrv)) {
			...
			ROS_ERROR("KCL: (ExportGenerator) Failed to call service %s", state_metric_service.c_str());
	...
	void ExportGenerator::printExpression(std::ofstream &pFile, rosplan_knowledge_msgs::ExprComposite & e) {
